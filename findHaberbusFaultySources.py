    ...
    def run(self):
        ...
        for data in self.newsSources.iteritems():
            ...
            for sourceList in sources:
                ...
                source = sourceList[0]
                if not self.serverHandler.executeQuery("SELECT id FROM `links_%s` WHERE date >= now() - interval 3 day and category='%s' and source='%s'" % (self.yearMonth, category, source)):
