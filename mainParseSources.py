    ...
    def getSocialCount(self, category, sourceTitle, returnLinkList, returnLinkDict):
        ...
        for link in returnLinkList:
            ...
            try:
                ...
                url = "https://clients6.google.com/rpc?key=AIzaSyCKSbrvQasunBoV16zDH9R33D88CeLr9gQ&quot"
                ...
                try:
                    ...
                    if imageId:
                        ...
                        if linkTitle:
                            ...
                            if not linkOrder:
                                self.serverHandler.executeQuery("INSERT INTO `links_%s` VALUES(NULL, CURRENT_DATE(), '%s', '%s', '%s', %d, %d, %d, %d, '%s', '%s', '%s',0, NULL, 1)" % (self.yearMonth, newsLinkDict[maxCountLink][4], newsLinkDict[maxCountLink][5], maxCountLinkInj, index, newsLinkDict[maxCountLink][1], newsLinkDict[maxCountLink][2], newsLinkDict[maxCountLink][3], linkTitle, linkDesc, imgSrc))
                                if self.showLink:
                                    self.logHandler.printMsg(maxCountLink, 3)
                            else:
                                #UpdateFlag 0 ise title, coutn guncellemeleri yapilmaz bu link icin
                                try:
                                    updateFlag = self.serverHandler.executeQuery("SELECT updateFlag FROM `links_%s` WHERE link='%s'" % (self.yearMonth, maxCountLinkInj))[0][0]
                                except:
                                    updateFlag = 1

                                if updateFlag:
                                    self.serverHandler.executeQuery("UPDATE `links_%s` SET `order`=%d, title='%s', description='%s', imgLink='%s' WHERE `link`='%s'" % (self.yearMonth, index, linkTitle, linkDesc, imgSrc, maxCountLinkInj))
    ...
    def run(self, category="", source=""):
        try:
            ...
            for data in self.newsSources.iteritems():
                ...
                #=======================================================
                # #Buldugun haber linklerini limit dogrultusunda database'e yaz
                #=======================================================
                databaseLinkLimit = 120
                if present.hour >= 15:
                    databaseLinkLimit = 10 * present.hour - 2

                self.insertToDatabase(category, newsLinkDict, databaseLinkLimit)
